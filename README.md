...
- **玩家缩放**：默认关闭，独立在“玩家缩放”配置分组中。开启后，玩家站在正式购物车中心区域一段时间会切换缩小/恢复；踩到车内物品、被货物短暂顶起或在车内短跳不会立刻重置，真正离开中心区域超过宽容时间后才会重置计时。关闭时 ShrinkCart 不运行玩家扫描、玩家缩放或玩家死亡保护逻辑。
...
R.E.P.O. 新版本的特殊物品同样可以被识别并缩小；v0.2.22 保留了 `CosmeticWorldObject` / `ItemValuableBox` 代币/外观箱适配。
...
- 购物车：启用购物车缩小、放入时缩小速度、取出后放大速度、离车防抖延迟、恢复后重新缩小冷却、保持原始重量、启用商店用品缩小、防止碰撞弹回原尺寸。
...
默认值：缩小速度 `0.5`，放大速度 `0.2`，离车防抖 `0.5`，恢复冷却 `0.5`，启用商店用品缩小 `false`，启用玩家缩放 `false`，玩家缩放倍率 `0.55`，玩家站车触发时间 `2` 秒，玩家离车判定宽容时间 `0.6` 秒，启用玩家受伤后自动恢复 `true`。车辆载物过滤固定为内部保护，不再作为配置项显示。贵重物倍率：Tiny `0.8`，Small `0.6`，Medium `0.45`，Big `0.4`，Wide `0.35`，Tall `0.35`，VeryTall `0.25`。敌人球倍率：Small `0.8`，Medium `0.65`，Big `0.45`，Berserker `0.45`。商店用品倍率 `0.5`，钱袋/Surplus `0.25`，代币/外观箱 `0.4`。

## 已知限制

- 缩放动画由 ScalerCore 控制，ShrinkCart 不重写底层缩放系统。
- ShrinkCart 不再接管车对车物理碰撞，也不会使用穿透修正或速度钳制；车辆真实碰撞表现以原版为准。
- ShrinkCart 不 patch 复活流程。若玩家缩放关闭且仍在 InstantRevive 复活后第二次死亡掉入虚空，请优先隔离测试 InstantRevive / RechargeableDefibro 等复活链路 mod。
...
ShrinkCart shrinks supported objects placed in carts, restores them after removal, keeps scaling host-authoritative through ScalerCore, keeps player scaling as an opt-in module, and leaves real cart-vs-cart collision behavior to vanilla physics.
...
- Player scaling is disabled by default and lives in its own config section. When disabled, ShrinkCart does not run player scanning, player scaling, or player death-safety logic.
